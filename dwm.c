...
static int vp;     // bar vertical padding
static int hp;     // bar horizontal padding
  ...
  if (ev->window == root) {
    ...
    if (updategeom() || dirty) {
      ...
      for (m = mons; m; m = m->next) {
        ...
        XMoveResizeWindow(dpy, m->barwin, m->wx + hp, m->by + vp,
                          m->ww - 2 * hp, bh);
      }
      focus(NULL);
      arrange(NULL);
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon || 1) {
    ...
    drw_text(drw, m->ww - tw - 2 * hp, 0, tw, bh, 0, stext, 0);
    // drw_text(drw, m->ww - sw - 2 * hp, vp, tw, bh, 0, stext, 0);
  }
  /*drw_setscheme(drw, scheme[SchemeNorm]);
  tw = TEXTW(stext) - lrpad + 2;
...
void togglebarpadding(const Arg *arg) {
  ...
  for (m = mons; m; m = m->next) {
    updatebarpos(m);
    XMoveResizeWindow(dpy, m->barwin, m->wx + hp, m->by + vp, m->ww - 2 * hp,
                      bh);
    arrange(m);
  }
}
...
void togglebar(const Arg *arg) {
  selmon->showbar = selmon->pertag->showbars[selmon->pertag->curtag] =
      !selmon->showbar;
  updatebarpos(selmon);
  XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + hp, selmon->by + vp,
                    selmon->ww - 2 * hp, bh);
  arrange(selmon);
}
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    m->barwin = XCreateWindow(
        dpy, root, m->wx + hp, m->by + vp, m->ww - 2 * hp, bh, 0,
